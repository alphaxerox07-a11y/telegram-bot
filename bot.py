...
# Apne Token aur URLs yahan daal lena (Agar pehle se upar likhe the toh unhe mat udana)
...
def process_btn_type(call):
    ...
    if call.data == "type_folder":
        ...
        msg = bot.send_message(
            chat_id,
            f"📝 Ab tum **Text, Photo, ya PDF File** bhejo jo **'{btn_name}'** par jayegi:",
            parse_mode="Markdown",
        )
...
def save_media_msg(message, btn_name):
    ...
    for user in all_users:
        try:
            bot.send_message(
                user["_id"],
                f"🔔 **NEW CHAPTER ADDED!**\n\n📚 Naya material **'{btn_name}'** upload kar diya gaya hai BBORON BOT me.\n\n👉 Jaldi se check karne ke liye /start par click karo!",
                parse_mode="Markdown",
            )
            success_count += 1
    ...
    bot.send_message(
        chat_id,
        f"📢 Broadcast Complete! Total {success_count} students ko DM update bhej diya gaya hai.",
        parse_mode="Markdown",
    )
        ...
        if msg_data:
            ...
            if sent_msg:
                warning_msg = bot.send_message(
                    chat_id,
                    "⏳ *Ye file 20 minute me auto-delete ho jayegi! Forward kar lo.*",
                    parse_mode="Markdown",
                )
...
# Agar tune keep_alive() alag file se import ki hai, toh wo rehne dena
try:
